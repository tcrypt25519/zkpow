...
pub fn maybe_dump_allocator_stats(path: &Path) {
    ...
    if let Err(err) = dump_allocator_stats(path) {
        tracing::warn!(path = %path.display(), error = %err, "failed to write jemalloc stats dump");
    } else {
        tracing::info!(path = %path.display(), "wrote jemalloc stats dump");
    }
}
...
fn snapshot_line(snapshot: &MemorySnapshot) -> String {
    ...
    if let Some(jemalloc) = snapshot.jemalloc {
        ...
        fields.push(format!("jemalloc_active_gap_bytes={}", jemalloc.active_gap_bytes()));
        fields.push(format!(
            "jemalloc_resident_gap_bytes={}",
            jemalloc.resident_gap_bytes()
        ));
        fields.push(format!("jemalloc_mapped_gap_bytes={}", jemalloc.mapped_gap_bytes()));
    }
...
/// Spawn a background thread that writes the process memory snapshot to `path`
